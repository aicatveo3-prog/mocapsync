...
**플랫폼: iOS (아이폰 11 × 2, iOS 17.5.1).** 안드로이드 앱은 `android/` 에 파킹되어 있습니다.

| 단계 | 내용 | 상태 |
| --- | --- | --- |
| **0** | PC에서 Pose2Sim 데모 실행 → `.trc` + `.mot` 생성 확인 | **완료** |
| **1.5** | **통신 규약 확정 + PC 테스트 마스터 + 슬레이브 시뮬레이터** | **완료** |
| 1' | iOS 프로젝트 골격 + macOS CI + 기기 진단 화면 | 진행 예정 |
| 2' | 클럭 동기 (성공 판정: 오차 상한 2ms 미만) | 규약/계산 검증 완료, 실기기 대기 |
| 3' | 녹화 + 예약 시작 + 타임스탬프 사이드카 + OIS 실측 | 대기 |
| 4 | 업로드 수신 + 키포인트 리샘플러 + Pose2Sim 자동 실행 | 대기 |
| 5 | 웹 3D 뷰어 / c3d 익스포트 + 품질 대시보드 | 대기 |
| 6 | 마스터 미리보기 그리드, 배터리/온도 모니터 | 대기 |
| 7 | (선택) 실시간 모드 — 영상 대신 키포인트만 전송 | 대기 |

> **1.5 단계를 1단계보다 먼저 한 이유**: 개발자가 Windows 에서 Swift 를 컴파일할 수
> 없습니다. 그래서 규약과 시각 계산을 **Python 으로 먼저 만들고 완전히 검증**한 뒤,
> iOS 앱이 "이미 검증된 상대"와 통신하게 만듭니다. 부수 효과로 폰 1대만으로도
> 클럭 동기 측정이 가능합니다. 자세한 내용은 [docs/PROTOCOL.md](docs/PROTOCOL.md).
