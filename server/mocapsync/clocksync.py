...
#
# 35% 차이. 원인: 120회 연사는 2초 만에 끝나므로 WiFi 상태의 **2초 창 하나**만
# 표본화합니다. 그 창이 나쁘면 최소 RTT 도 나쁩니다. 같은 2초 안에서 왕복을
# 1000회로 늘려도 창이 바뀌지 않으니 소용없습니다.
#
# 필요한 것은 여러 시간 창을 보는 것인데, 그냥 간격을 주면 무선이 매번 절전에
# 들어가 손해입니다. 둘을 동시에 만족시키는 방법이 버스트입니다.
#   · 버스트 내부는 연사     -> 무선이 깨어 있음
#   · 버스트 사이는 휴식     -> 다른 시간 창을 봄
#   · 버스트마다 짧은 워밍업 -> 휴식 뒤 절전 기동시간을 본 표본에서 제외

#: 버스트 하나에 넣을 측정 왕복 수. 0 이면 단일 연사.
DEFAULT_BURST_SIZE = 20

#: 버스트 사이 휴식(ms)
DEFAULT_BURST_GAP_MS = 300

#: 각 버스트 시작 시 버리는 왕복 수
DEFAULT_BURST_WARMUP = 3
